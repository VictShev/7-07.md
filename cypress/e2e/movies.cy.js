...
describe('Main Page', () => {
    ...
    it('Should display main page correctly', () => {
        ...
        cy.get(':nth-child(1) > .movie__info > .movie__description')
        cy.get('main > :nth-child(1) > :nth-child(2)')
        cy.get('main > :nth-child(1) > :nth-child(3)')
...
cy.get(`.buying-scheme__wrapper > :nth-child(${seat.row}) > :nth-child(${seat.seat})`).click();
 ...
 it("Should be possible to book", () => {
    cy.visit('http://qamid.tmweb.ru')
    cy.get('a.page-nav__day:nth-of-type(4)')
    cy.get('.movie').first().contains('20:00').click();
